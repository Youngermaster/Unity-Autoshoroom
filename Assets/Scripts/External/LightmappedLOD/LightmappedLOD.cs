...
public class LightmappedLOD : MonoBehaviour
{
    ...
    void RendererInfoTransfer()
    {
        ...
        {
            Debug.LogWarning("Lightmapped LOD : lod index not found on " + gameObject.name);
            return;
        }
            ...
            if (renderers[i] != null)
            {
                ...
                catch
                {
                    if (Debug.isDebugBuild)
                        Debug.LogError("Lightmapped LOD : Error setting lightmap settings on " + gameObject.name);
                }
            }
        }
    }
